package com.example.servicemanager;

public class Constants {
	public static final String IMEIs = "863122031636606, 863122031636614, 356044061622070, 353918053946568";
	public static final String FILE_NAME = "Data.csv";
	public static final String FOLDER_NAME = "Service Manager";
	public static final String ADD_CUSTOMER = "Add Customer";
	public static final String SEARCH_CUSTOMER = "Search Customer";
	public static final String DUE_SERVICES = "Due Services";
	public static final String UPCOMING_SERVICES = "Upcoming Services";
	public static final String IMPORT = "Import";
	public static final String CALLED_FROM = "calledFrom";
	public static final String CUSTOMER = "CUSTOMER";
	public static final String DB_NAME_WITH_EXTENTION = "MyDBName.db";
	public static final String DB_NAME = "MyDBName";
	public static final String SERVICE_NO = "Service No";
	public static final String SERVICE_DATE = "Service Date";
}
