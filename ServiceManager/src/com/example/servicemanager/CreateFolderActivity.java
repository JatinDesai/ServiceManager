...
public class CreateFolderActivity extends SyncWithDrive {
  ...
  public void onConnected(Bundle connectionHint) {
    ...
    MetadataChangeSet changeSet = new MetadataChangeSet.Builder().setTitle(EXISTING_FOLDER_ID).build();
    Drive.DriveApi.getRootFolder(getGoogleApiClient()).createFolder(
        getGoogleApiClient(), changeSet).setResultCallback(folderCreatedCallback);
  };

  final ResultCallback<DriveFolderResult> folderCreatedCallback = new ResultCallback<DriveFolderResult>() {
    @Override
